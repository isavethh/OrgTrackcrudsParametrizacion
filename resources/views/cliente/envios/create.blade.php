...
@section('page-content')
                        ...
                        <div class="card h-100">
                            ...
                            <div class="card-body">
                                ...
                                <div class="alert alert-info mt-3 text-sm">
                                    ...
                                    Si seleccionas una ruta guardada, el mapa se centrará automáticamente. Si marcas puntos
                                    nuevos, se creará una nueva dirección.
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="col-md-8">
                        <div class="card h-100">
                            ...
                            <div class="card-body p-0">
                                <div id="mapNuevoEnvio" style="height: 500px; width: 100%; position: relative; z-index: 1;">
                                </div>
                            </div>
                        </div>
                    </div>
    ...
    <template id="tplParticion">
        <div class="card card-outline card-primary mb-4 particion-item" data-index="{index}">
            <div class="card-header">
                <h3 class="card-title"><i class="fas fa-truck mr-2"></i> Envío / Camión #<span
            ...
            <div class="card-body collapse show" id="collapsePart{index}">
                ...
                <div class="row">
                    <div class="col-md-4">
                        <div class="form-group">
                            ...
                            <div class="clickable-input-wrapper" onclick="this.querySelector('input').showPicker();"
                ...
                <div class="row">
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <textarea class="form-control js-instr-recogida" rows="2"
                                placeholder="Ej: Puerta trasera, preguntar por..."></textarea>
                        </div>
                    </div>
                    <div class="col-md-6">
                        <div class="form-group">
                            <label>Instrucciones de Entrega</label>
                            <textarea class="form-control js-instr-entrega" rows="2"
                                placeholder="Ej: Dejar en recepción..."></textarea>
                        </div>
                    </div>
                </div>
                ...
                <button type="button" class="btn btn-sm btn-outline-success btn-add-carga mt-2">
        ...
        <div class="card card-outline card-secondary mb-3 carga-item">
            ...
            <div class="card-body">
                ...
                <h6 class="text-info"><i class="fas fa-ruler-combined mr-2"></i>Tamaño / Conteo <span
                        class="text-danger">*</span></h6>
                <small class="text-muted d-block mb-2">La etiqueta de la caja indica cuántas unidades contiene (ej: 40, 75,
                    100, 198, 216)</small>
                <div class="row">
                    <div class="col-md-4">
                        <div class="form-group">
                ...
                <!-- Forma de Pedir y Cantidad -->
                <h6 class="text-info mt-3"><i class="fas fa-pallet mr-2"></i>Forma de Pedir y Cantidad <span
                        class="text-danger">*</span></h6>
                <small class="text-muted d-block mb-2">Puede pedir por <strong>empaques</strong> (cajas/bolsas/pallets) o
                    por <strong>número de unidades</strong> (ej: 1000 manzanas)</small>
                <div class="row">
                    <div class="col-md-4">
                        <div class="form-group">
                    ...
                    <div class="col-md-4">
                        <div class="form-group">
                            <label>Cantidad de Pedido <span class="text-danger">*</span></label>
                            <input type="number" class="form-control form-control-sm js-cantidad-pedido" min="1" required>
                            <small class="text-muted">Ej: 10 cajas o 1000 unidades</small>
                        </div>
                    </div>
                    <div class="col-md-4">
                        <div class="form-group">
                ...
                <input type="hidden" class="js-cantidad-calculada">
                <input type="hidden" class="js-peso-calculado">
            </div>
        </div>
    </template>
...
@push('js')
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            loadTiposTransporte();
            loadCatalogs().then(() => {
                addPartition(); // Agregar primera partición después de cargar catálogos
            });
            ...
            async function loadCatalogs() {
                try {
                    ...
                    console.log('Catálogos cargados:', { categorias: state.categorias.length, productos: state.productos.length });
            ...
            function loadCatalogOptions(card) {
                ...
                function populateConteos(productId) {
                    ...
                    const conteosFiltrados = state.tamanosConteo.filter(item => !productId ? false : (item.id_producto == productId || item.id_producto == null));
                ...
                selectTipoEmpaque.addEventListener('change', function () {
                    ...
                    if (sel.value) {
                        card.querySelector('.js-largo').value = sel.getAttribute('data-largo') || '';
                        card.querySelector('.js-ancho').value = sel.getAttribute('data-ancho') || '';
                        card.querySelector('.js-alto').value = sel.getAttribute('data-alto') || '';
                        card.querySelector('.js-tara').value = sel.getAttribute('data-tara') || '';
                        card.querySelector('.js-unidades-pallet').value = sel.getAttribute('data-unidades-pallet') || '48';
                        const cap = sel.getAttribute('data-capacidad');
                        if (cap) card.querySelector('.js-capacidad-empaque').value = cap;
                ...
                selectCategoria.addEventListener('change', function () {
                    ...
                    const selectProd = card.querySelector('.js-carga-producto');
                    selectProd.innerHTML = '<option value="">Seleccione...</option>';
                    if (idCat) {
                        state.productos.filter(p => p.id_categoria === idCat).forEach(prod => {
                            const opt = document.createElement('option');
                            opt.value = prod.id;
                ...
                card.querySelector('.js-carga-producto').addEventListener('change', function () {
                    const sel = this.options[this.selectedIndex];
                    populateConteos(this.value);
                    if (sel.value) {
                        card.querySelector('.js-peso-promedio').value = sel.getAttribute('data-peso-promedio') || '';
                ...
                selectConteo.addEventListener('change', function () {
                    const sel = this.options[this.selectedIndex];
                    if (sel.value) {
                        const peso = sel.getAttribute('data-peso');
                        if (peso) card.querySelector('.js-peso-promedio').value = peso;
                        card.querySelector('.js-capacidad-empaque').value = sel.value;
                        calcularTotales(card);
                    }
                });

                card.querySelector('.js-forma-pedido').addEventListener('change', () => calcularTotales(card));
            ...
            function calcularTotales(card) {
                ...
                let empaquesCalculados = 0, cantidadTotal = 0;
                if (formaPedido === 'unidades' && conteo > 0) {
                ...
                const numeroPallets = (unidadesPallet > 0 && empaquesCalculados > 0) ? Math.ceil(empaquesCalculados / unidadesPallet) : 0;

                card.querySelector('.js-peso-neto').value = pesoNeto.toFixed(2);
                card.querySelector('.js-peso-bruto').value = pesoBrutoTotal.toFixed(2);
                card.querySelector('.js-empaques-calculados').value = empaquesCalculados;
                card.querySelector('.js-numero-pallets').value = numeroPallets;
                card.querySelector('.js-cantidad-calculada').value = cantidadTotal;
                card.querySelector('.js-peso-calculado').value = pesoBrutoTotal.toFixed(2);
            }
            ...
            function getFormData() {
                ...
                document.querySelectorAll('.particion-item').forEach(p => {
                    ...
                    p.querySelectorAll('.carga-item').forEach(c => {
                        ...
                        const cantidadCalculada = parseFloat(c.querySelector('.js-cantidad-calculada')?.value) || 0;
                        const pesoCalculado = parseFloat(c.querySelector('.js-peso-calculado')?.value) || 0;
                        const empaquesCalculados = parseInt(c.querySelector('.js-empaques-calculados')?.value) || 0;
                        const productoSelect = c.querySelector('.js-carga-producto');
                        const productoNombre = productoSelect?.selectedOptions[0]?.textContent || 'Producto';

                        cargas.push({
                            id_categoria: idCategoria ? parseInt(idCategoria) : null,
                            id_producto: idProducto ? parseInt(idProducto) : null,
                            id_tipo_empaque: idTipoEmpaque ? parseInt(idTipoEmpaque) : null,
                            cantidad: cantidadCalculada || empaquesCalculados,
                            peso: pesoCalculado,
