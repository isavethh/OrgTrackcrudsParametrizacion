...
<script>
    ...
    async function guardarTransportista(e){
        ...
        let payload = {
            ci: fieldCI.value.trim(),
            telefono: fieldTelefono.value.trim()
        };

        if (!payload.ci || !payload.telefono){
        ...
        if (id){
            ...
            if (chkNuevoUsuario.checked){
                if (!fieldNombre.value.trim() || !fieldApellido.value.trim() || !fieldCorreo.value.trim() || !fieldContrasena.value.trim()){
                    errorForm.textContent = 'Completa los datos del nuevo usuario.';
                    errorForm.classList.remove('d-none');
                    return;
                }
                payload = {
