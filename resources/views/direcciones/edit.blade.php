...
@section('title', 'Editar Dirección/Ruta')
...
@section('content')
    <div class="card">
        <div class="card-body">
            <form action="{{ route('direcciones.update', $direccione) }}" method="POST" id="direccionForm">
                @csrf
                @method('PUT')
                ...
                <div class="form-group">
                    ...
                    <select name="envio_id" id="envio_id" class="form-control @error('envio_id') is-invalid @enderror" required>
                        ...
                        @foreach($envios as $envio)
                            <option value="{{ $envio->id }}" {{ old('envio_id', $direccione->envio_id) == $envio->id ? 'selected' : '' }}>
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="nombre_ruta" id="nombre_ruta" class="form-control @error('nombre_ruta') is-invalid @enderror" 
                        value="{{ old('nombre_ruta', $direccione->nombre_ruta) }}" required>
                    @error('nombre_ruta')
                        <span class="invalid-feedback">{{ $message }}</span>
                    @enderror
                ...
                <input type="hidden" name="ruta_geojson" id="ruta_geojson" value="{{ old('ruta_geojson', $direccione->ruta_geojson) }}">
...
@section('js')
    ...
    <script>
        ...
        @if($direccione->ruta_geojson)
            try {
                const existingRoute = {!! $direccione->ruta_geojson !!};
                if (existingRoute && existingRoute.coordinates && existingRoute.coordinates.length > 0) {
                    // Dibujar ruta
                    routeLayer = L.geoJSON(existingRoute, {
                        style: {
                            color: '#3388ff',
                            weight: 5,
                            opacity: 0.7
                    ...
                    const start = existingRoute.coordinates[0];
                    const end = existingRoute.coordinates[existingRoute.coordinates.length - 1];
                    
                    origenCoords = { lat: start[1], lng: start[0] };
                    destinoCoords = { lat: end[1], lng: end[0] };
                    
                    // Crear marcadores
                    origenMarker = L.marker([origenCoords.lat, origenCoords.lng], {
                        icon: L.icon({
                    ...
                    destinoMarker = L.marker([destinoCoords.lat, destinoCoords.lng], {
                        icon: L.icon({
                    ...
                    document.getElementById('origen_lat').value = origenCoords.lat.toFixed(8);
                    document.getElementById('origen_lng').value = origenCoords.lng.toFixed(8);
                    document.getElementById('destino_lat').value = destinoCoords.lat.toFixed(8);
                    document.getElementById('destino_lng').value = destinoCoords.lng.toFixed(8);
                    
                    // Obtener nombres
                    reverseGeocode(origenCoords.lat, origenCoords.lng).then(nombre => {
                        document.getElementById('origen_nombre').value = nombre;
                    });
                    reverseGeocode(destinoCoords.lat, destinoCoords.lng).then(nombre => {
                ...
                console.error('Error al cargar ruta existente:', e);
            }
        @endif
