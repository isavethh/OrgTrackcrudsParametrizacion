            ...
            <form action="{{ route('admins.store') }}" method="POST">
                ...
                <div class="form-group">
                    ...
                    <select name="nivel_acceso" id="nivel_acceso" class="form-control @error('nivel_acceso') is-invalid @enderror" required>
                        ...
                        <option value="1" {{ old('nivel_acceso') == '1' ? 'selected' : '' }}>Nivel 1 - Básico</option>
                        <option value="2" {{ old('nivel_acceso') == '2' ? 'selected' : '' }}>Nivel 2 - Intermedio</option>
                        <option value="3" {{ old('nivel_acceso') == '3' ? 'selected' : '' }}>Nivel 3 - Avanzado</option>
                        <option value="4" {{ old('nivel_acceso') == '4' ? 'selected' : '' }}>Nivel 4 - Supervisor</option>
                        <option value="5" {{ old('nivel_acceso') == '5' ? 'selected' : '' }}>Nivel 5 - Administrador</option>
                    </select>
                    @error('nivel_acceso')
                        <span class="invalid-feedback">{{ $message }}</span>
                    @enderror
