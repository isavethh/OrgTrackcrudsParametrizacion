...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::table('envios', function (Blueprint $table) {
            $table->foreignId('id_tipo_vehiculo')->nullable()->after('id_direccion')->constrained('tipos_vehiculo')->nullOnDelete();
            $table->foreignId('id_transportista_asignado')->nullable()->after('id_tipo_vehiculo')->constrained('usuarios')->nullOnDelete();
            $table->string('estado_aprobacion')->default('pendiente')->after('estado_tracking'); // pendiente, aprobado, rechazado
